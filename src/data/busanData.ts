export type BusanSpotKey = '서면' | '광안리' | '해운대' | '기장' | '남포동' | '기타(동네)';
...
export const COMMON_LOCATION_SUGGESTIONS: string[] = ['서면', '광안리', '해운대', '남포동', '기장', '영도', '기타(동네)'];

export const getInsight = (location: string | null | undefined) => {
  const normalized = (location ?? '').trim();
  const exists = Boolean(normalized && Object.prototype.hasOwnProperty.call(BUSAN_SPOT_INSIGHTS, normalized));
  const key: BusanSpotKey = exists ? (normalized as BusanSpotKey) : '기타(동네)';
